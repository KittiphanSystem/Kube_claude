...
// ตัวแปร global flags
var (
	repoPath string // path ของ platform-gitops
	autoPush bool   // auto git push หลังสร้าง tenant
)
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for _, t := range tenants {
				// อ่าน tenant.yaml เพื่อดู plan/domain
...
func getTenantInfo(repoPath, name string) tenantInfo {
	// อ่านค่าจาก tenant.yaml (simplified)
	...
	// Simple parse (ไม่ใช้ full YAML parser เพื่อลด dependency)
	content := string(data)
...
// ============================================================
// tenant sync (regenerate bootstrap จาก tenant.yaml)
// ============================================================
...
func newTenantSyncCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// TODO: implement full sync from tenant.yaml
